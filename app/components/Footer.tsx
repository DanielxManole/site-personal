...
  // 🎯 NOU: Referință pentru a preveni dubla execuție (Mouse + Touch)
  const touchHandledRef = useRef(false);

  // Funcție unificată pentru interacțiune (Mouse și Touch)
  const handleInteraction = (e: React.MouseEvent | React.TouchEvent) => {
    
    // Dacă este un eveniment de mouse și știm că a fost deja tratat de touch, ignoră-l.
    if (e.type === 'click' && touchHandledRef.current) {
        touchHandledRef.current = false; // Resetează flag-ul după o scurtă perioadă
        return;
    }
...
  // 🎯 Funcții simple pentru a seta flag-ul de touch
  const handleTouchStart = () => {
    // Setează flag-ul pentru a ignora evenimentul de mouse care va urma
...
                  onClick={handleInteraction}
                    // 🎯 NOU: Adaugă handlerele tactile
                    onTouchStart={handleTouchStart}
                    onTouchEnd={handleInteraction}
                    // NOTĂ: Am lăsat onClick activ ca fallback pentru desktop/mouse
                  className={`
